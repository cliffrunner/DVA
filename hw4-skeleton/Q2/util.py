...
def entropy(class_y):
    ...
    counts = class_counts(class_y)
    for key in counts.keys():
        prob = counts[key]/len(class_y)
...
def partition_classes(X, y, split_attribute, split_val):
    ...
    if type(X[0][split_attribute])==str:
        X_left = [_ for _ in X if _[split_attribute]==split_val]
        X_right = [_ for _ in X if _[split_attribute]!=split_val]
        y_left = [_ for i, _ in enumerate(y) if X[i][split_attribute]==split_val]
...
def information_gain(previous_y, current_y):
    ...
    info_gain = entropy(previous_y) - probl*entropy(current_y[0]) - probr*entropy(current_y[1])
    return info_gain
...
def find_best_split(X,y):
    ...
    for i, col in enumerate(range(n_features)):
        ...
        for val in values:
            X_left, X_right, y_left, y_right = partition_classes(X, y, i, val)
